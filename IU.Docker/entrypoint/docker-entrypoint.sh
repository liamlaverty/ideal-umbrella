...
if [ "$1" = '/opt/mssql/bin/sqlservr' ]; then
  ...
  if [ ! -f /tmp/app-initialized ]; then
    ...
    function initialize_app_database() {
      ...
      # These variables are passed in from dockercompose, via dockerfile.mssql
      ...
      /opt/mssql-tools/bin/sqlcmd -S $2 -U $3 -P $4 -d master -i docker-umbraco-setup.sql -v UMBRACO_DB_NAME="$5" UMBRACO_DB_USER_LOGIN="$6" UMBRACO_DB_USER_PASSWORD="$7" UMBRACO_DB_USER_NAME="$6_USER"
      # /opt/mssql-tools/bin/sqlcmd -S $2 -U $3 -P $4 -d master -i docker-umbraco-setup.sql
